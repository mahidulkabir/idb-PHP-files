...
class FileUploadHandler {
    ...
    public function isValidFile() {
        return in_array($this->file_type, $this->allowed_types) && ($this->file_size / 1024) < 1000;
    }
...
class StudentDataClass extends FileUploadHandler {
    ...
    public function __construct($id, $name, $course, $phone, $email, $file_input) {
        parent::__construct($file_input); // handle file
        $this->id = $id;
        $this->name = $name;
        $this->course = $course;
        $this->phone = $phone;
    ...
    public function save() {
        if ($this->saveFile()) {
            ...
            echo " File not valid or upload failed.";
        }
    }
